...
function [spectra, freqs, time_windows] = calculate_fft_spectrum(rr_interp, config)
    % Konstanten aus Config extrahieren
    ...
    % Hanning-Fenster zur Unterdrückung von Leck-Effekten (Spectral Leakage)
    win = hann(window_length);
    
    % Spektrogramm berechnen (Signal Processing Toolbox)
    ...
    % Die Toolbox liefert P als [Frequenz x Zeit]. Um eure bisherigen 
